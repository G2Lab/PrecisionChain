...
'''
createStream-StructuralVariants.py
Creates streams for the structural variant view of the genetic data in the blockchain
Usage: $ python createStream-StructuralVariant.py -cn=<chain name> -dr=<Chain path> 
...
def createStreams(chainName, multichainLoc, datadir):
    ...
    makeStream(chainName, "mappingData_variants_structural", multichainLoc, datadir) #header, other things for the whole file
   
    #data stream  split by chromosome
    for i in range(1, 23):
        makeStream(chainName, "structural_chrom_{}".format(i), multichainLoc, datadir)
    
    #data stream for sex chromosomes
    makeStream(chainName, "structural_chrom_x", multichainLoc, datadir)  
    makeStream(chainName, "structural_chrom_y", multichainLoc, datadir) 
    
    for i in range(1, 23):
        makeStream(chainName, "person_structural_chrom_{}".format(i), multichainLoc, datadir) 
    
    for i in range(1, 23):
        makeStream(chainName, "MAF_structural_chrom_{}".format(i), multichainLoc, datadir) 
    
    makeStream(chainName, "MAF_structural_chrom_x", multichainLoc, datadir)  
    makeStream(chainName, "MAF_structural_chrom_y", multichainLoc, datadir) 
    return
